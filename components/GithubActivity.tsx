...
// A small palette so each language gets a recognizable dot. Anything not listed
// falls back to the teal accent.
  ...
  try {
    const res = await fetch(
      `https://api.github.com/users/${GITHUB_USERNAME}/repos?per_page=100&sort=pushed`,
      {
        ...
        // Revalidate hourly so we stay fresh without hammering GitHub's
...
export default async function GithubActivity() {
  ...
  // If GitHub is unreachable or rate-limited, skip the section entirely rather
  // than rendering an empty or broken block.
