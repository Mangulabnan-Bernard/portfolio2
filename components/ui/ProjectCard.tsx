...
export default function ProjectCard({ project }: ProjectCardProps) {
  const cardClass = project.featured
    ? 'project-card border border-teal/25 bg-linear-to-br from-teal/4 to-surface rounded-[14px] p-7 flex flex-col lg:col-span-3 transition-colors hover:border-teal/40 hover:-translate-y-0.5'
    ...
    <div className={cardClass}>
      ...
      <div className="project-tech flex flex-wrap gap-2 mb-5">
        {project.tech.map((tag) => (
          <span
            key={tag}
            className="tech-tag font-mono text-[9px] tracking-[0.08em] px-2 py-1 bg-dim border border-border text-muted rounded-[4px] uppercase"
          >
            {tag}
          </span>
        ))}
